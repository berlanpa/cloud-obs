...
import GradientEarth from '../public/background-images/gradient-eart.jpg';
import GradientMars from '../public/background-images/gradient-mars.jpg';

// Background image paths
const BACKGROUND_IMAGES = [
  { name: 'Gradient Earth', path: GradientEarth },
  { name: 'Gradient Mars', path: GradientMars },
];
...
export function CameraSettings() {
  ...
  // Initialize background type based on current track processor
  React.useEffect(() => {
    if (cameraTrack && isLocalTrack(cameraTrack.track)) {
      const processorName = cameraTrack.track.getProcessor()?.name;
      if (processorName === 'background-blur') {
        setBackgroundType('blur');
      } else if (processorName === 'virtual-background') {
  ...
  const camTrackRef: TrackReference | undefined = React.useMemo(() => {
    if (!cameraTrack || !localParticipant) return undefined;
  ...
  React.useEffect(() => {
    if (cameraTrack && isLocalTrack(cameraTrack.track)) {
      try {
        ...
        console.error('Error setting track processor:', error);
      }
    }
  }, [cameraTrack, backgroundType, virtualBackgroundImagePath]);

  // Don't render if we don't have a room context
  if (!localParticipant) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <p>Camera settings will be available when you join a meeting.</p>
      </div>
    );
  }
